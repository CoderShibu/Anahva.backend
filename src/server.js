...
try {
  db.exec(`CREATE TABLE IF NOT EXISTS Session (session_id TEXT PRIMARY KEY);`);
  db.exec(
    `CREATE TABLE IF NOT EXISTS Journal (id TEXT PRIMARY KEY, session_id TEXT, encrypted_payload TEXT, allow_ai_memory INTEGER, created_at TEXT, updated_at TEXT, deleted_at TEXT);`,
  );
  if (
    !db
      .prepare("SELECT 1 FROM Session WHERE session_id = ?")
      .get("demo-session")
  ) {
    db.prepare("INSERT INTO Session (session_id) VALUES (?)").run(
      "demo-session",
    );
  }
} catch (e) {}
...
const callGemini = async (message, mode) => {
  ...
  // ONLY USE MODELS CONFIRMED AVAILABLE
  const attempts = [
    // Primary: Gemini 2.5 Flash (Stable)
    `/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GOOGLE_API_KEY}`,
  ];
  ...
  throw (
    lastError ||
    new Error("All endpoints failed. Verified models: gemini-2.5-flash")
  );
};
...
app.post("/api/chat/message", async (req, res) => {
  try {
    ...
    res
      .status(503)
      .json({ error: "AI Error", message: error.message, stack: error.stack });
  }
>>>>>>> 94cf55708ecf343c1f89d2c455aff79f76dfe894
});
...
app.post("/api/journal/create", (req, res) => {
  try {
    const id = randomUUID();
    db.prepare(
      "INSERT INTO Journal (id, session_id, encrypted_payload, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).run(
      id,
      "demo-session",
...
app.get("/api/journal/list", (req, res) => {
  const journals = db
    .prepare(
      "SELECT * FROM Journal WHERE session_id='demo-session' AND (deleted_at IS NULL OR deleted_at='')",
    )
    .all();
  res.json({ success: true, journals });
>>>>>>> 94cf55708ecf343c1f89d2c455aff79f76dfe894
...
app.post("/api/journal/analyze", async (req, res) => {
  try {
    ...
    // Prompt for Analysis
    let prompt =
      "Analyze the mental health progress based on these journal entries. Return strictly valid JSON array of objects: [{ date: 'YYYY-MM-DD', score: number (0-100 indicating wellness), sentiment: 'Positive'|'Neutral'|'Negative' }]. Do not include markdown formatting or explanations. detailed JSON only.\n\nEntries:\n";
...
app.delete("/api/journal/clear", (req, res) => {
  try {
    const result = db
      .prepare(
        "UPDATE Journal SET deleted_at = ? WHERE session_id = 'demo-session' AND (deleted_at IS NULL OR deleted_at = '')",
      )
...
app.delete("/api/journal/:id", (req, res) => {
  db.prepare("UPDATE Journal SET deleted_at=? WHERE id=?").run(
    new Date().toISOString(),
    req.params.id,
  );
  res.json({ success: true });
...
app.post("/api/auth/demo", (req, res) =>
  res.json({ token: "demo", session_id: "demo-session", success: true }),
);
